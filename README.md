...
Every _version range_ contains one or more _sets of comparators_. To satisfy _version range_ version must satisfies all _comparators_ from at least one set. Sets are separated with two vertical bars `||`. Rules in each set are separated with whitespaces. Empty set are treated as wildcard comparator `*`.
